...
class RQAI:
    ...
    oQDirectory = (
        "Organized_Questions"  # Folder that contains the sorted questions file.
    )
    ...
    keyLocks = (
        {}
    )  # Locks to protect the system from multiple programs accessing the same critical section at the same time.
    ...
    def __init__(self):
        ...
        )  # Creating the communication queue for use between main program and the Daemon.
        self.connectionQ = (
            q  # Save connection with Daemon object to class level variable.
        )
        with open(os.path.join(f"{self.sDirectory}", f"{self.sfileCat}")) as file:
            categories = file.readline().split(", ")
        self.categories = (
            categories  # Categories generated from the above accessed ini file.
        )
        self.nlp = spacy.load(
            "en_core_web_sm"
        )  # The core module that the spacy library uses to run in this program.
        # Below this line used for monitoring the progress of the RQAI initialization
        count = 0
        for _ in os.listdir(
            self.cDirectory
        ):  # Gets a count of the total number of files in the cDirectory
            count += 1
        if (
            count == 0
        ):  # Warning messages assosciated with potential problems in the cDirectory1
            ...
            categories
        ):  # Note: The self.build method could be removed above to potentially cut down on load times for the RQAI, but hasn't been found to impact load times dramatically and so has been included by default.
    ...
    def AITraining(self, arg=""):
        ...
        keyMap = (
            {}
        )  # The key map is a list of the number of files in the cDirectory with a number assigned to it to use for the human entry
        count = 0
        ...
        for filename in os.listdir(
            self.qDirectory
        ):  # Capable of reading multiple different files in the Questions directory.
            qContents = []
            tempContents = []
            questionsCount += 1
            if (
                arg != "build"
            ):  # Part of TODO: enable the AI to make decisions automatically.
                print(
                    f"Would you like to train with {filename}? (Y/n)?", end=" "
                )  # Enables the user to choose for each file whether or not to train with that file.
                if input().lower().__contains__("y"):
                    with open(
                        os.path.join(self.qDirectory, filename), "r"
                    ) as file:  # Reads through qDirectory chosen file and commits it to local memory
                        qContents = file.readlines()
                    for (
                        question
                    ) in (
                        qContents
                    ):  # Reads through EVERY LINE in the questions document and asks the user to make decisions based off of those lines.
                        defintionSet = self.GetQuestionSet(
                            question
                        )  # Gets the words that go into the category files, these words are the definitions of the subjects of the question.
                        guess = self.GuessQuestion(
                            defintionSet
                        )  # Makes a guess looking for similar words in the category files and then returns the highest match.
                        guessLocation = keyMap.get(
                            guess
                        )  # Used for UI coloring, the guessLocation is the order that the location file appears in the file
                        print("    ", end="")  # tab
                        print(
                            f"{question.strip()}"
                        )  # Prints the question that the program is currently examining without the \n
                        print("    ", end="")  # tab
                        print(
                            f"This looks like a \033[1;{(guessLocation % 6) + 31};40m{guess}\033[1;37;40m question. Press enter to input the question into the database as a {guess} question or enter a number for a different question type:"
                        )  # AI guess
                        types = ""
                        ...
                        print(
                            types
                        )  # Prints out the keymap with the according file names as the options for recording the question.
                        print(">>> ", end="")  # UI
                        entry = ""
                        ...
                        if uI == "":  # If UI blank:
                            ...
                            while loop:  # Until good input is reached:
                                try:
                                    uI = int(
                                        uI
                                    )  # Tries to get an integer from user input.
                                    entry = list(keyMap.keys())[
                                        list(keyMap.values()).index(uI)
                                    ]  # Tries to get the assosciated key from user input.
                                    loop = False
                                except:
                        ...
                        tempContents.append(
                            question
                        )  # Adds the question to tempContents to use with removing questions from the qDirectory accessed file
                        with open(f"{self.oQDirectory}/{self.oQuestionsTxt}", "r") as f:
                            contents = (
                                f.readlines()
                            )  # Get the contents of the organized question file
                        if contents.__contains__(
                            f"{entry}:\n"
                        ...
                        contents.insert(
                            contents.index(f"{entry}:\n") + 1, question
                        )  # Inserts new question after the category heading
                        ...
                        for item in contents:  # Pulls every question from oQuestions.
                            _tempContents[
                                _tempContents.index(item)
                            ] = item.strip()  # Remove any newlines
                        ...
                        with open(f"{self.oQDirectory}/{self.oQuestionsTxt}", "w") as f:
                            contents = "\n".join(
                                contents
                            )  # Joines newline-less lines with newlines before writing contents to file.
                            f.write(contents)  # Writes contents to file
                        self.AddCategoriesTxt(
                            defintionSet, entry
                        )  # Adds the definition set that was used earlier to the category files that were assigned by the entry
                for (
                    item
                ) in (
                    tempContents
                ):  # Takes any questions that the user read from the questions file and removes them from the read file in the qDirectory
                    qContents.remove(item)
                file = open(
                    os.path.join(self.qDirectory, filename), "w"
                )  # Opens the qDirectory accessed file with write permissions
    ...
    def Build(self):
        ...
        for filename in os.listdir(
            self.cDirectory
        ):  # For every file in the cDirectory:
        ...
        for (
            ...
            self.categories
        ):  # For every category_type in the local categories variable:
            f = os.path.join(self.cDirectory, type)
            if os.path.isfile(
                f"{f}.txt"
            ):  # If the file from the local list exists in the directory already.
                message = f"{f} file exists."  # Print that the file exists.
                RQAIGUI.printOk(message)  # Print the RQAIGUI OK message.
                files.remove(
                    f"{type}.txt"
                )  # Remove from the list of files that are still needed.
            else:  # The file doesn't exist:
                open(f"{f}.txt", "x")  # Creates the file.
                file = open(
                    f"{f}.txt", "w"
                )  # Populates the document with the needed header format.
                file.write(
                    "[RQAIV]:0"
                )  # RQAI Value Keeps track of total words in document.
        ...
        if addCount != 0:  # If any files were added
            print(
                f"{addCount} file(s) added in total to training list."
            )  # Prints the total number of lists that were made.
            ...
            while loop:  # Prevent bad input
                try:
                    ...
                    if uI.__contains__("y"):  # If user input contains 'y':
                        os.remove(
                            f"{file}"
                        )  # Remove the file from the categories folder
                        rmvCount += 1
        ...
        if rmvCount != 0:  # If any files were removed:
            print(
                f"{rmvCount} file(s) removed in total from training list."
    ...
    def CheckCategory_Types(self):
        if os.path.isfile(
            os.path.join(self.sDirectory, self.sfileCat)
        ):  # If path is valid:
    ...
    def GetQuestionSet(self, question):
        questionNP = question.translate(
            str.maketrans("", "", string.punctuation)
        )  # Removes punctuation from question
        dict = Dictionary  # Generates the local dictionary to be used.
        definitionSet = {"init"}  # Frequency of words in definition is omitted.
        definitionSet.remove(
            "init"
        )  # Removes the supplied word so that data structure is recognized as a set and is empty.
        subjects = self.GetSubj(questionNP)  # Gets the subjects of question.
        for word in subjects:  # For every word in the subjects list:
            dict = Dictionary(
                word, 1
            )  # Dictionary definition for each word in subject.
            for i in (
                dict.meanings()[0]
                .translate(str.maketrans("", "", string.punctuation))
                .split(" ")
            ):  # Splits definition by spaces and removes all punctuation so that "the" and "the." are the same.
                if (len(i) > 2) & (
                    (i != "and") & (i != "the")
                ):  # Removes small words like "if" and "of" as well as "and" and "the".
    ...
    def GuessQuestion(self, definitionSet):
        matchPercentage = (
            {}
        )  # Holds both the category types as well as the percentage to how similar the definitionSet is to the category.
        for filename in os.listdir(self.cDirectory):  # For every file in the cDirectory
            path = os.path.join(self.cDirectory, filename)
            with open(
                f"{path}", "r"
            ) as f:  # Open the path to the selected file in the cDirectory.
                contents = (
                    f.readlines()
                )  # Copies the contents of the files to a new local instance
                ...
                for val in definitionSet:  # for every value in the definitionSet:
                    dSet = self.BstIndex(
                        contents, val
                    )  # Lookup of the val from definitionSet to see if the value is in the file
                    (
                        found,
                        position,
                    ) = dSet.items()  # Unpacks the dictionary that was returned
                    if found[1]:  # Checks "found" value from the dictionary
                        amount += int(
                            contents[position[1]].strip().split(":")[1]
                        )  # Item was found, add the number of word occurences to amount.
                totalWordCount = int(
                    contents[0].split(":")[1]
                )  # TotalWordCount is [RQAIV] val
                if (
                    totalWordCount == 0
                ):  # The document has no words so don't make a divide by 0 error:
                    matchPercentage[filename.split(".")[0]] = 0  # Return 0
                else:  # The document is populated so apply the formula:
                    matchPercentage[filename.split(".")[0]] = (
                        100 * amount / totalWordCount
                    )  # The percentage to which the file contents match the definitionSet contents.
        ...
        print(
            self.MakeChart(matchPercentage, colors)
        )  # Prints a graphical representation of how the AI is making its decisions
        for (
            key,
            val,
        ) in (
            matchPercentage.items()
        ):  # For every value in the matchPercentage definitionSet:
            if (
                val >= highestVal
            ):  # If value is greater than or equal to highestKey replace highest with these values.
                highestKey = key  # Update highestKey
                highestVal = val  # Update highestVal
        return (
            highestKey  # Returns the category which matches the definitionSet the best.
        )
    ...
    def GetSubj(
        self, question
    ):  # The method to get the subjects using the Spacy module.
        qnlp = self.nlp(
            question
        )  # Instantiate the Spacy nlp instance with provided question.
        nSubjArray = [
            nSubj for nSubj in qnlp if (nSubj.dep_ == "nsubj")
        ]  # add nSubj to the nSubjArray if part of speech in qnlp is defined as a "nsubj".
    ...
    def AddCategoriesTxt(self, dictOutput, category, signature="sys"):
        ...
        dt_string = dateAndTime.strftime(
            "%Y.%m.%d.%H.%M.%S"
        )  # Reformats the date and time in a way that's more useful for the RQAI.
        with open(
            f"{self.cDirectory}/{category}.txt", "r"
        ) as f:  # Opens the document for the specified category.
            contents = (
                f.readlines()
            )  # Copies document contents for the specified category from the document to a local variable.
        ...
        for item in contents:  # For every line in contents
            wordContents.append(
                item.split(":")[0]
            )  # Builds wordcontents as per the above defined requirements.
            tempContents[
                tempContents.index(item)
            ] = item.strip()  # Removes any newlines from tempContents.
            if item == "":  # Stop buildup of newlines
                tempContents.remove(
                    item
                )  # Removes any blank lines which were previously just a \n
        ...
        for word in dictOutput:  # For every word in the provided definition set:
            lookupDict = self.BstIndex(
                contents, word
            )  # A set containing the lookup information that's created by BstIndex.
            found, position = lookupDict.items()  # Unpack the BstIndex set.
            if found[1]:  # If item was found:
                signatureOld = contents[position[1]].split(":")[
                    2
                ]  # Gets the old signature.
                if (
                    signature == signatureOld
                ):  # If both entries were made by the same author:
                    timeSignatures = int(
                        contents[position[1]].split(":")[1]
                    )  # Gets the number of time signatures.
                    ...
                    contents[
                        position[1]
                    ] = f"{contents[position[1]].split(':')[0]}:{(int(contents[position[1]].strip().split(':')[1]).__add__(1))}:{signatureOld}:{time}"  # Increases the item's value by one.
                else:  # The entries were made by different authors:
                    spot = (
                        position[1] + 1
                    )  # Checks spots starting after the current position.
                    added = False  # Variable to keep track of whether or not an addition has been made.
                    for i in range(
                        2
                    ):  # Checks for multiple different signatures, since only three are available (man, sys, ans) there are only two checks left to make.
                        if spot + i < len(
                            contents
                        ):  # If we haven't gone past the length of the contents array
                            if (contents[spot + i].split(":")[0] == word) & (
                                contents[spot + i].split(":")[2] == signature
                            ):  # If the line word and signature match the current word and signature
                                timeSignatures = int(
                                    contents[spot + i].split(":")[1]
                                )  # Gets the number of time signatures.
                                time = ""  # Instantiate time variable
                                for t in range(
                                    timeSignatures
                                ...
                                contents[
                                    spot + i
                                ] = f"{contents[spot + i].split(':')[0]}:{(int(contents[spot + i].strip().split(':')[1]).__add__(1))}:{signature}:{time}"  # Increases the item's value by one.
                                added = True  # A value was added.
                    if (
                        not added
                    ):  # No value has been added so add it at the found position.
                        contents.insert(
                            position[1], f"{word}:1:{signature}:{dt_string}"
                        )  # Inserts new item at appropriate location.
            else:  # Item wasn't found:
                if position[1] >= len(
                    contents
                ):  # If location is at the end of the list:
                    contents.append(
                        f"{word}:1:{signature}:{dt_string}"
                    )  # Append rather than insert to avoid errors.
                else:  # Item wasn't found at the end of the list
                    if (
                        contents[position[1]].split(":")[0] < word
                    ):  # The item is at the end of the list but position couldn't be moved to the end of the list.
                        contents.append(
                            f"{word}:1:{signature}:{dt_string}"
                        )  # Append rather than insert to avoid errors.
            ...
            contents[
                0
            ] = f"{contents[0].split(':')[0]}:{(int(contents[0].split(':')[1]).__add__(1))}"  # Updates the [RQAIV] information
        with open(
            f"{self.cDirectory}/{category}.txt", "w"
        ) as f:  # Opens the category file with write permissions:
    ...
    def MakeChart(self, matchPercentage, colors, size=10):
        width = (
            len(matchPercentage) * 2 + 3
        )  # The width is the measurement of the blank space at the beginning of the line until the newline.
        lines = []  # A blank array to store our lines.
        lines.append(
            "      " + "".join(["_" for _ in range(len(matchPercentage) * 2)]) + "_\n"
        )  # Header
        values = list(
            matchPercentage.values()
        )  # Gets the values from our matchPercentage dictionary.
        for i in range(size):  # Body
            ...
            if i > 0:  # Check to make sure we don't divide by 0.
                linePercent = int(
                    100 / size * (size - i)
                )  # Assignment of the percentage indicator.
            ...
            while (
                len(line) <= 4
            ):  # If line length is not already 4 (length of characters "---%")
            ...
            for i in range(width - 2):  # Width without the "|" characters
                if i % 2 == 1:  # If at an even spot
                    if (
                        values[int(i / 2)] >= linePercent
                    ):  # If spot value for category is greater than or equal to the current line percent indicator:
                        line += (
                            colors[int(i / 2)] + "x" + "\033[1;37;40m"
                        )  # Mark a colored "x".
        ...
        lines.append(
            ("0%   |")
            + "".join(["_" for _ in range(len(matchPercentage) * 2)])
            + "_|\n"
        )  # Footer
        keys = list(
            matchPercentage.keys()
        )  # List of the keys (names) in the matchPercentage variable
            ...
            try:  # Tries to add labels from the document, adding line will fail if any document name is shorter in length than three letters.
                ...
                for c in range(width - 3):  # width without "|" characters and offset:
                    if c % 2 == 0:  # If at an even spot:
                        line += (
                            colors[int(c / 2)] + keys[int(c / 2)][i] + "\033[1;37;40m"
                        )  # Adds character from category document name.
                    else:  # At an odd spot:
    ...
    def BstInternal(
        self, array, val, bottom, top, endCondition
    ):  # Efficiency is more complex than the overview given in BstIndex, some repeat values are allowed which means that the number of calls could exceed what were listed above by a factor of three (for each signature type), but this would be rare.
        pos = int(
            (top + bottom) / 2
        )  # Position is located at the middle of the document
        if (
            abs(top - bottom) <= endCondition
        ):  # If remaining document size is less than or equal to the endcondition value:
            ...
            for i in range(len(array) - bottom):  # For currently defined bottom to top.
                ...
                if array[pos].split(":")[0] == (val):  # Value is in document:
                    ...
                    val < array[pos].split(":")[0]
                ):  # Value is now less than read value:
                    break  # End search.
            dict[
                "Position"
            ] = pos  # Position information where spot was found or passed.
        ...
        if array[pos].split(":")[0] == (val):  # Item was found during Bst splits
            ...
            return self.BstInternal(
                array, val, bottom, pos - 1, endCondition
            )  # Read everything above (lower than) the current value
        else:  # Document value is less than currently read value:
            return self.BstInternal(
                array, val, pos + 1, top, endCondition
            )  # Read everything below (higher than) the current value
    ...
    def Rollback(
        self, target, dateTime, signature="all"
    ):  # This method will take a long time to run!
        ...
        for line in contents:  # For every line in the contents file:
            ...
            items = line.split(
                ":"
            )  # Divides the line into an array by splitting around the ':' character
            itemsRemoved = 0  # Variable to keep track of the number of items deleted from the items array
            if (
                items[2] == signature
            ) | signature == "all":  # If the targeted signature is found
            ...
            for i in range(
                len(items) - 3
            ):  # A loop to check through all the time signatures.
                if (
                    items[i + 2] > dateTime
                ):  # Checks to see if item dateTime is larger than the dateTime specified (needs to be removed).
                    itemsRemoved = (
                        len(items) - 3 - i
                    )  # The number of items removed is equal to the total number items minus the first three, minus the for loop iteration.
                    items[i + 2] = "ROLLBACK"  # Marks the spot for removal.
            items[1] = (
                int(items[1]) - itemsRemoved
            )  # Changes the number of occurences of the word.
            contents[
                0
            ] = f"{contents[0].split(':')[0]}:{int(contents[0].split(':')[1]) - itemsRemoved}"  # Updates the RQAI value to reflect the number of items that were removed.
            tempItems = []  # Spot to hold the new line in separate array indices.
            for i in range(len(items)):  # For every spot in the items array
                if (
                    items[i] != "ROLLBACK"
                ):  # If the spot is not marked for rollback then continue appending items onto the array:
                    tempItems.append(items[i])  # append the item.
                else:  # The spot is marked for rollback
                    break  # Stop the loop to add new items.
            newLine = ":".join(
                tempItems
            )  # Joins the tempItems array together into a new line.
    ...
    def Restore(self):
        print(
            "Enter the date to restore at in a YYYY.MM.DD format:", end=" "
        )  # Asks for data in a year/month/day format.
        date = input()  # Takes user input
        sizeOf = len(
            date.split(".")
        )  # Retrieve the size of the format, as long as at least a year, month, and day were included the program can continue.
        if sizeOf >= 3:  # Check to make sure that UI is in a format that can be used.
            ...
            folderName = ".".join(
                fArray
            ).strip()  # Makes a folder name to check for out of user input without a newline.
            backupsList = os.listdir(
                self.bDirectory
            )  # Array of folders in the backup directory.
            if backupsList.__contains__(
                folderName
            ):  # If folders has a folder that matches the user input.
                print(
                    f"Match found at \033[1;32;40m{self.bDirectory}\\{folderName}\033[1;37;40m"
                )  # Print information about the match.
                with open(
                    os.path.join(self.bDirectory, self.bFileLog), "r"
                ) as file:  # Open the backup_log file
                    contents = file.readlines()  # Copies the contents to a local array
                indexOf = (
                    contents.index(folderName + "\n") + 1
                )  # Finds the first occurence of the folder matching the user input.
                items = (
                    []
                )  # Blank array for the items that will be found under the folder name.
                bytes = 0  # Total bytes that the operation will involve.
                while (
                    contents[indexOf]
                    .split(" ")[len(contents[indexOf].split(" ")) - 1]
                    .strip()
                    == "bytes"
                ):  # Every file line ends in "bytes".
                    bytes += int(
                        contents[indexOf].split(":")[1].split(" ")[0]
                    )  # Adds the backupfile_log byte information to the total number of bytes the operation will involve.
                    print(
                        f"    {contents[indexOf].strip()}"
                    )  # Print the file information after a tab and without the extra \n.
                    ...
                    if indexOf + 1 > len(
                        contents
                    ):  # Check to make sure that the file length isn't exceeded.
                        break  # File length exceeded: Break.
                print(
                    "Are you sure you want to restore these files? (Y/n)?", end=" "
                )  # Double check these are the fiels the user wants to restore.
                ...
                while loop:  # Until good input is reached:
                    if uI.lower().__contains__("y"):  # If user input is "y"
                        ...
                        for (
                            index
                        ) in items:  # For every index listed in the items array:
                            if os.path.exists(
                                os.path.join(
                                    self.cDirectory,
                                    contents[index].split(".")[0] + ".txt",
                                )
                            ):  # If file exists:
                                os.remove(
                                    os.path.join(
                                ...
                                )  # Remove the corresponding file from the cDirectory
                            if sys.platform.__contains__(
                                "win"
                            ):  # This is a windows system and requires windows commands:
                                os.popen(
                                    f"copy {os.path.join(self.bDirectory, folderName, contents[index].split(':')[0])} {os.path.join(self.cDirectory, contents[index].split('.')[0])}.txt"
                                )  # Copy file using Windows commands.
                            else:  # System is a unix system:
                                os.popen(
                                    f"cp {os.path.join(self.bDirectory, folderName, contents[index].split(':')[0])} {os.path.join(self.cDirectory, contents[index].split('.')[0])}.txt"
                                )  # Copy file using Unix commands.
                        print(
                            f"Files successfully restored. \033[1;32;40m{bytes}\033[1;37;40m bytes written."
                        )  # Prints status information for the operation.
                    elif uI.lower().__contains__(
                        "n"
                    ):  # Received good user input stating to break:
                        loop = False  # Stops the loop to keep asking for user input.
                    else:  # User input was bad.
                        print(
                            "Are you sure you want to restore these files? (Y/n)?",
                            end=" ",
                        )  # Reprompts user input.
...
class RQAIDaemon:
    ...
    def __init__(self, cDirectory, connectionQ, bDirectory, bFileLog):
        ...
        self.connectionQ = (
            connectionQ  # Copy the RQAI connectionQ object to Daemon's memory
        )
        ...
        self.backupMaxSize = (
            ...
            000,
            000,
        )  # 25 Megabytes is the current size allotted to the backupMax folder before it will engage the pruning algorithm.
        ...
        for filename in os.listdir(
            self.cDirectory
        ):  # Loads the files in the cDirectory and commits them to local memory.
            currentFile += 1  # Adds one to the loading variable for the current number of the file's load order
            if self.VerifyFileIntegrity(
                filename
            ):  # Reading the RQAI Files and committing them to local memory
                self.connectionQ.put(
                    f"File verified ({currentFile}/{count})"
                )  # UI information
    ...
    def Pruning(self):  # In progress
        ...
        if totalSize > self.backupMaxSize:
            for folder in os.listdir(
                self.bDirectory
            ):  # Sorts items in backup folders into a single dictionary for checking.
                for item in folder:
                    if not dictionary.__contains__(item.split(".")[0]):
                        dictionary[item.split(".")[0]] = []
                    dictionary[item.split(".")].append(
                        os.path.join(self.bDirectory, folder, item)
                    )
        for key in list(
            dictionary.keys()
        ):  # Access every dictionary key with its attached list of items:
            for item1 in dictionary.get(
                key
            ):  # for every array in the dictionary: (n^2 loop to check items)
                with open(item1) as file1:
                    contents1 = file1.readline()
                for item2 in dictionary.get(
                    key
                ):  # Checks against every array except for itself:
                    if item2 != item1:
                        with open(item2) as file2:
                            contents2 = file2.readline()
                        largerVal = int(
                            contents1[0].split(":")[1]
                        )  # Assign the larger RQAIV val to largerVal
                        smallerVal = int(
                            contents2[0].split(":")[1]
                        )  # Assign the smaller RQAIV val to smallerVal
                        ...
                        if (
                            largerVal * 0.99 <= smallerVal
                        ):  # If smallerValue is 90% of largerValue continue check:
                            set1, set2 = set()
                            ...
                            if len(
                                set1.union(set2) == lenOriginal
                            ):  # No new words have been added, only new occurences, discard old information:
    ...
    def BackupData(self):
        if not (
            os.path.exists(os.path.join(self.bDirectory, self.bFileLog))
        ):  # Checks to see if log file path exists:
            open(
                os.path.join(self.bDirectory, self.bFileLog), "x"
            )  # If file doesn't exist creates the file.
            message = f"{self.bFileLog} was created successfully."  # Output message
            self.connectionQ.put(
                message
            )  # Sends a message that the file was created to the multiprocessing queue.
        ...
        for item in os.listdir(self.cDirectory):  # For every item in the cDirectory
            ...
            with open(
                os.path.join(self.cDirectory, item)
            ) as file:  # Opens the subject file for reading
                contents = file.readlines()  # Copies the file contents to an array
            try:
                if (
                    contents[0].split(":")[2] == "verified"
                ):  # Attempts to read a spot that may not exist
                    backupItem = (
                        contents[0].strip().split(":")[3].split(".")
                    )  # Gets the verification date
                    fArray = [backupItem[0], backupItem[1], backupItem[2]]
                    folderName = ".".join(
                        fArray
                    )  # Constructs folder name from verification info
                    newFileName = f"{item.split('.')[0]}.{'.'.join(fArray)}.txt"
                    if not (
                        os.path.isdir(os.path.join(self.bDirectory, folderName))
                    ):  # If the folder for the day doesn't already exist:
                        os.mkdir(
                            os.path.join(self.bDirectory, folderName)
                        )  # Make the folder.
                    if not os.path.exists(
                        os.path.join(self.bDirectory, folderName, newFileName)
                    ):  # If the backup item doesn't already exist
                        open(
                            os.path.join(self.bDirectory, folderName, newFileName), "x"
                        )  # Create the backup item
                        with open(
                            os.path.join(self.bDirectory, folderName, newFileName), "w"
                        ) as newFile:  # Open the newly created backup item
                            newFile.writelines(
                                contents
                            )  # Write the old file contents to a new file. TODO: Copy file instead of copying contents
                        with open(
                            os.path.join(self.bDirectory, self.bFileLog), "r"
                        ) as log:  # Opens the system log file.
                            contents = (
                                log.readlines()
                            )  # Copies the log files into a local array.
                        if contents.__contains__(
                            folderName + "\n"
                        ):  # Searches through the array to find if the folder exists already:
                            indexOf = contents.index(
                                folderName + "\n"
                            )  # Folder already exists, grabs its index.
                            if not (
                                indexOf + 1 >= len(contents)
                            ):  # Make sure that we aren't trying to add data to a spot that doesn't exist.
                                contents.insert(
                                    indexOf + 1,
                                    f"{newFileName}:{f'{os.path.getsize(os.path.join(self.bDirectory, folderName, newFileName))}'} bytes",
                                )  # Insert new item into the backup file.
                            else:  # We're at the end of the list:
                                contents.append(
                                    f"{newFileName}:{f'{os.path.getsize(os.path.join(self.bDirectory, folderName, newFileName))}'} bytes"
                                )  # Appends rather than inserts the new value.
                        else:  # The folder entry doesn't already exist:
                            contents.append(
                                folderName
                            )  # adds the folder name to the file.
                            contents.append(
                                f"{newFileName}:{f'{os.path.getsize(os.path.join(self.bDirectory, folderName, newFileName))}'} bytes"
                            )  # Appends the entry onto the file.
                        newContents = (
                            []
                        )  # The new contents that will be written to the log file.
                        for (
                            item
                        ) in contents:  # For every item in the log file contents.
                            newContents.append(
                                item.strip() + "\n"
                            )  # Make sure there's a newline at the end.
                        with open(
                            os.path.join(self.bDirectory, self.bFileLog), "w"
                        ) as log:  # Opens the system log file
                            log.writelines(
                                newContents
                            )  # Write the new contents to the log file.
            except (
                IndexError
            ) as e:  # Catch previously defined error of accessing a spot that may not exist.
                self.connectionQ.put(
                    f"Index error was reached : {e}"
                )  # File wasn't ready to be read, add the error to the multiprocessing shared memory object.
    ...
    def VerifyFileIntegrity(self, filename):
        ...
        if os.path.isfile(
            f
        ):  # Checks to see if file at filename is file (it had better be)
            with open(f) as file:  # If that file exists opens it.
                contents = (
                    file.readlines()
                )  # Copies the file contents to a local variable.
            total = 0  # Total number of entries for the file.
            flag = False  # Flag to let the user know if anything unexpected was encountered.
            if not (
                contents[0].split(":")[0] == "[RQAIV]"
            ):  # Checks to make sure header is correct:
                flag = True  # If not correct flip flag.
            for i in range(
                len(contents) - 1
            ):  # For every item in contents besides the header:
                try:
                    spot = (
                        i + 1
                    )  # Starts at the first spot and updates every item after.
                    tempContents = contents[spot].split(
                        ":"
                    )  # Copies the line into a local array.
                    size = int(
                        tempContents[1]
                    )  # The size is found in the second array indice.
                    total += size  # Total is increased by size.
                    for c in range(size):  # Loop for every expected date item.
                        tempContents[
                            c + 3
                        ] = "Accessed"  # States that the item was read correctly.
                    for c in range(len(tempContents) - 3):  # Loop for every date item.
                        if (
                            tempContents[c + 3] != "Accessed"
                        ):  # If item hasn't been accessed:
                            flag = True  # Flips the flag.
                except (
                    IndexError
                ):  # Catches IndexErrors caused by accessing items that don't exist (size was too large)
                    flag = True  # Flips the flag.
            if total != int(
                contents[0].split(":")[1]
            ):  # If total doesn't match the RQAIV total:
                flag = True  # Flips the flag.
            if flag:  # If the flag has been flipped at any point:
                ...
                with open(f, "w") as file:  # Open file with read permissions.
                    rqaivLine = contents[0].split(
                        ":"
                    )  # Split the contents by the ':' character.
                    dateAndTime = (
                        datetime.now()
                    )  # The time when this method was called.
                    dt_string = dateAndTime.strftime(
                        "%Y.%m.%d.%H.%M.%S"
                    )  # Reformats the date and time in a way that's more useful for the RQAI.
                    if len(rqaivLine) > 2:  # If the heading information already exists:
                        rqaivLine[
                            2
                        ] = "verified"  # Reupdate the header in array indice[2].
                        rqaivLine[3] = (
                            dt_string + "\n"
                        )  # Reupdate the header in array indice[3].
                    else:  # The header doesn't exist:
                        rqaivLine[1] = rqaivLine[
                            1
                        ].strip()  # Clean the newline off of rqaivLine[1].
                        rqaivLine.append(
                            "verified"
                        )  # Add the header information to array indice[2].
                        rqaivLine.append(
                            dt_string + "\n"
                        )  # Add the header information with formatting to array indice[3].
                    contents[0] = ":".join(
                        rqaivLine
                    )  # Join the contents together using RQAI encoding.
                    contents = ("").join(
                        contents
                    )  # Joins all the contents together into a single string.
                    file.writelines(
                        contents
                    )  # Writes all the lines of the edited contents array to the file.
