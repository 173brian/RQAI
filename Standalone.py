...
for module, reqVersion in requirements.items():
    ...
    try:
        ...
        if (userVersion < reqVersion) & (userVersion > 0):
            print(
                f"User version is {userVersion} which is less than the required version of {reqVersion}. Update now? (Y/n)?",
                end=" ",
            )
        ...
        if str(userVersion) < reqVersion:
            print(
                f"RQAI requires the {module} with version {reqVersion} to run. Download now? (Y/n)?",
                end=" ",
            )
            userInput = input()
...
if (
    __name__ == "__main__"
):  # Check to ensure that only the main thread can make a daemon as a new process.
    _RQAI = None
    arguments = sys.argv
    ...
    )  # Creates the daemon as a new process.
    ...
    while True:
        ...
        if userInput.lower().__contains__("help"):
            ...
            while not _RQAI.connectionQ.empty():
                print(
                    f"\033[1;36;40mDaemon completed tasks:\033[1;37;40m {_RQAI.connectionQ.get()}"
                )
